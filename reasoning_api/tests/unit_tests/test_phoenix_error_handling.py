...
class TestPhoenixErrorHandling:
    ...
    def test__phoenix_unavailable__api_continues_working(self, caplog: pytest.LogCaptureFixture) -> None:
        """Test API continues working when Phoenix service is unavailable."""
        with caplog.at_level(logging.WARNING), mock_phoenix_unavailable():
    ...
    def test__api_endpoints_work_without_tracing(self) -> None:
    ...
    def test__streaming_works_without_phoenix(self) -> None:
        """
    ...
    def test__invalid_phoenix_config__fails_gracefully(self) -> None:
...
class TestPhoenixRecovery:
    ...
    def test__tracing_can_be_re_enabled_after_failure(self, phoenix_sqlite_test: str) -> None:  # noqa: ARG002
        """Test that tracing can be re-enabled after initial failure."""
        # First, simulate Phoenix unavailable
        with mock_phoenix_unavailable(), pytest.raises(Exception):  # noqa: PT011
            setup_tracing(
                enabled=True,
    ...
    def test__mixed_tracing_states__handled_correctly(self, phoenix_sqlite_test: str) -> None:  # noqa: ARG002

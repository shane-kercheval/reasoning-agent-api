...
class TestTracingIntegration:
    ...
    def test__tracing_disabled__reasoning_works(self) -> None:
        """Test that reasoning works correctly when tracing is disabled."""
        with mock_settings(enable_tracing=False):
    ...
    def test__tracing_enabled__reasoning_still_works(self) -> None:
    ...
    def test__health_check__works_with_tracing(self) -> None:
    ...
    def test__middleware__adds_tracing_headers(self) -> None:
...
class TestTracingFunctional:
        ...
        Usage:
            def test_example(self, mock_litellm_with_response):
                ...
                with mock_litellm_with_response(chunks):
                    ...
                    # ... test code ...
        """
        def _create_mock(chunks: list[ChatCompletionChunk]):
    ...
    def test__chat_completion_creates_traces__sqlite(self, tracing_enabled, mock_litellm_with_response):  # noqa
        """Verify chat completion creates traces in SQLite Phoenix database."""
        # Define mock response chunks
        chunks = [
            ChatCompletionChunk(
                id="chatcmpl-test",
                choices=[Choice(index=0, delta=ChoiceDelta(content="Hello! I'm doing well.", role="assistant"), finish_reason=None)],
                created=1234567890,
                model="gpt-4o-mini",
                object="chat.completion.chunk",
            ),
        ...
        with patch('reasoning_api.main.setup_tracing'):
            # Set up litellm mock
            with mock_litellm_with_response(chunks):
    ...
    def test__streaming_chat_completion_with_tracing__works(self, tracing_enabled, mock_litellm_with_response):  # noqa
    ...
    def test__tool_calling_with_tracing_enabled__functional_validation(self, tracing_enabled, mock_litellm_with_response):  # noqa
        """Verify tool calling works correctly when tracing is enabled."""
        # Define mock response chunks with tool result content
        chunks = [
            ChatCompletionChunk(
                id="chatcmpl-test",
                choices=[Choice(index=0, delta=ChoiceDelta(content="The weather in Paris is 22°C and partly cloudy.", role="assistant"), finish_reason=None)],
                created=1234567890,
                model="gpt-4o-mini",
                object="chat.completion.chunk",
            ),
    ...
    def test__api_health_check_with_tracing__always_works(self) -> None:
        ...
        ), disable_authentication(), TestClient(app) as client:
            response = client.get("/health")
            assert response.status_code == 200
            assert "status" in response.json()
    ...
    def test__reasoning_steps_traced__light_validation(self, tracing_enabled):  # noqa

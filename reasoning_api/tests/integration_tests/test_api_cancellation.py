...
from reasoning_api.main import chat_completions
from reasoning_api.openai_protocol import OpenAIChatRequest
from reasoning_api.context_manager import ContextManager
...
class TestExecutorCancellation:
    ...
    async def test_disconnection_stops_stream(
        ...
        patch_target = (
            "reasoning_api.executors.passthrough.litellm.acompletion"
            if executor_mode == "passthrough"
            else "reasoning_api.executors.reasoning_agent.litellm.acompletion"
        )

        with patch("reasoning_api.main.verify_token", return_value=True), \
    ...
    async def test_span_marked_on_cancellation(
        ...
        with patch("reasoning_api.main.tracer.start_span", return_value=mock_span), \
             patch("reasoning_api.main.verify_token", return_value=True), \
...
class TestReasoningAgentCancellationSpecifics:
    ...
    async def test_cancellation_during_reasoning_loop(
        ...
        with patch("reasoning_api.main.verify_token", return_value=True), \
             patch("reasoning_api.executors.reasoning_agent.litellm.acompletion",

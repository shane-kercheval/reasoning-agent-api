...
# 8. Reload environment variables: docker compose up -d
...
Integration tests use **testcontainers** to automatically start LiteLLM stack:

```bash
# Integration tests auto-start containers - no manual setup needed!
make integration_tests
```

**How it works**:
1. `testcontainers-python` library starts docker-compose stack before tests
2. LiteLLM proxy + PostgreSQL spin up automatically
3. Test fixtures wait for services to be healthy
4. Tests run against real LiteLLM proxy
5. Containers are automatically cleaned up after tests

**Benefits**:
- ✅ No manual docker-compose commands needed
- ✅ Works in CI/CD without setup steps
- ✅ Tests verify full production path (through LiteLLM proxy)
- ✅ Automatic cleanup after test completion

**Requirements**:
- Docker daemon must be running
- `OPENAI_API_KEY` environment variable must be set (used by LiteLLM to call OpenAI)
...
Integration tests require LiteLLM proxy to be running manually:

```bash
# Start LiteLLM stack only (faster than full stack)
docker compose up -d litellm postgres-litellm

# Set environment variables (use test key for separate usage tracking)
export OPENAI_API_KEY=<your_openai_key>
export LLM_BASE_URL=http://localhost:4000
export LLM_API_KEY=<your_litellm_test_key>  # Or use master key for quick testing

# Run integration tests (will use LiteLLM proxy)
make integration_tests
```

**Key Points:**
- Tests connect to `http://localhost:4000` (LiteLLM proxy)
- LiteLLM proxy forwards requests to OpenAI with observability
- Use `LITELLM_TEST_KEY` in .env for separate usage tracking from dev/eval
- Master key (`LITELLM_MASTER_KEY`) can be used for quick testing
- All integration test fixtures now use LiteLLM instead of direct OpenAI

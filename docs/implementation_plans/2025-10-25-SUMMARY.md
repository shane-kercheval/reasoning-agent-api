...
### 1. Backend: LiteLLM Models Proxy (Quick, ~1 day)
...
### 2. Backend: Conversation Storage (Medium, ~1-2 weeks)
**Plan**: `docs/implementation_plans/2025-10-25-conversation-storage.md`
- 6 milestones (DB setup, CRUD, API changes, endpoints, testing)
- New postgres instance (postgres-reasoning)
- Smart stateful/stateless hybrid approach
- Required for desktop client Milestone 6+

### 3. Desktop Client: Electron Migration (Main Work, ~3-4 weeks)
...
- Start backend conversation storage immediately (Milestones 1-4)
- Start desktop client Milestones 1-5 in parallel
- Desktop client Milestone 6 blocks on backend completion
- Enables parallel development without idle time

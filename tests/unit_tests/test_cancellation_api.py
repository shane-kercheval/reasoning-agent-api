...
class TestCancellationAgentInterface:
    """
    Test agent cancellation interface contracts.

    PORTABLE: These tests verify any agent supports cancellation.
    They test the interface, not implementation details.

    Tests focus on:
    - Agent execute_stream() cancellation support
    - CancelledError propagation through agent
    - Agent resource cleanup on cancellation
    """
    ...
    async def test_cancelled_error_propagation(
        ...
        """
        Test agent interface handles CancelledError propagation correctly.
        ...
        # Create mock request that never reports disconnection
        mock_request = AsyncMock(spec=Request)
        mock_request.headers = {"user-agent": "test"}
        mock_request.url = MagicMock()
        mock_request.url.__str__.return_value = "http://test/v1/chat/completions"
    ...
    @pytest.mark.asyncio
    async def test_agent_execute_stream_supports_cancellation(
        self,
        chat_request: OpenAIChatRequest,
        any_agent: AsyncMock,
    ) -> None:
        """
        Test agent.execute_stream() handles task cancellation properly.
        ...
        # Test agent directly, not through API
    ...
    @pytest.mark.asyncio
    async def test_agent_cleanup_on_cancellation(
        self,
        chat_request: OpenAIChatRequest,
        any_agent: AsyncMock,
    ) -> None:
        """
        Test agent cleans up resources when cancelled.
        ...
        # Start and cancel the agent task
        stream_task = asyncio.create_task(consume_with_cleanup())
        ...
        assert cleanup_called, "Agent should clean up resources on cancellation"

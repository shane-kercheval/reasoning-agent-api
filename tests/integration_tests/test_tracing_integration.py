...
class TestTracingFunctional:
    ...
    @pytest.fixture
    def mock_openai_client_with_response(self):
        """
        Factory fixture for mocking OpenAI client with custom response chunks.

        Returns a function that takes chunks and sets up dependency override.
        Automatically handles cleanup via yield.

        Usage:
            def test_example(self, mock_openai_client_with_response):
                chunks = [ChatCompletionChunk(...), ...]
                mock_openai_client_with_response(chunks)
                # Dependency override is active
                # ... test code ...
                # Cleanup happens automatically after test
        """
        def _create_mock(chunks: list[ChatCompletionChunk]) -> AsyncMock:
            """Create mock client with specified response chunks."""
            ...
            # Override the dependency to use our mocked client
    ...
    def test__chat_completion_creates_traces__sqlite(self, tracing_enabled, empty_mcp_config: str, mock_openai_client_with_response):  # noqa
        ...
        # Set up mock client with dependency override (cleanup automatic)
        mock_openai_client_with_response(chunks)
    ...
    def test__streaming_chat_completion_with_tracing__works(self, tracing_enabled, empty_mcp_config: str, mock_openai_client_with_response):  # noqa
    ...
    def test__tool_calling_with_tracing_enabled__functional_validation(self, tracing_enabled, empty_mcp_config: str, mock_openai_client_with_response):  # noqa
